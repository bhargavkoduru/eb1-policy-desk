...
PASSAGE = {'id': 'eb1b-p019-c01', 'document_id': 'eb1b', 'source_pdf_page': 19,
           'source_url': 'https://www.uscis.gov/policy-manual/volume-6-part-f-chapter-3',
           'section': 'Judging', 'text': 'The beneficiary must actually participate in judging.'}
CLAIM = {'text': 'Document completed judging.', 'citations': [PASSAGE['id']],
...
def test_question_submission_displays_cited_answer(monkeypatch):
    def fake_ask(retriever, question, category):
        return {'status': 'answered', 'question': question, 'category': category, 'claims': [CLAIM],
                'passages': [PASSAGE], 'seconds': 0.1, 'retrieval_warning': None}
    monkeypatch.setattr(eb1.qa, 'ask', fake_ask)
    st.cache_resource.clear()
    app = AppTest.from_file(str(Path(__file__).resolve().parents[1] / 'app.py'), default_timeout=20).run()
    app.text_area[0].set_value('What judging evidence is needed?')
    button(app, 'Find a cited answer').click().run()
    assert not app.exception
    assert any('PDF p. 19' in m.value for m in app.markdown)
    st.cache_resource.clear()

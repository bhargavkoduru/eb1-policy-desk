...
flowchart TD
    ...
    UI --> AG[Week 3 LangGraph research agent]
...
The Week 2 and Week 3 modes are independently accessible in the sidebar. Week 3 also invokes retrieval directly: users do not have to ask a Week 2 question first.
...
The local Streamlit process binds to `127.0.0.1`. Questions and policy excerpts go to Nebius. SQLite checkpoints and approved checklists stay on the local computer and persist across app restarts. See the README for setup.

...
st.title('EB-1 Policy Desk')
st.write('Explore EB-1A and EB-1B policy, check the sources, and save a reviewed research checklist.')
...
if mode.startswith('Week 2'):
    st.subheader('Ask a policy question')
    st.caption('Example: How do EB-1A and EB-1B differ on job offers?')
    with st.form('qa'):
    ...
    if 'last_answer' in st.session_state:
        ...
        st.markdown(answer_markdown(result))
        st.caption(f"{result['status'].capitalize()} · {result['seconds']:.1f} seconds · citations link to live chapters; PDF page numbers refer to your saved snapshot.")
        if result.get('retrieval_warning'):
            st.warning(result['retrieval_warning'])
        for n, claim in enumerate(result.get('claims', []), 1):
            with st.expander(f'Evidence for statement {n}'):
